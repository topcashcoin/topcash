...
uint256 hashGenesisBlock("0x000001c6e8d73093d36057f3a46de1b685c6f176cafc7d2a632a68e03fec52a1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TopCash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TopCash Signed Message:\n";
...
{
    // TopCash: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // TopCash: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // 
		...
		const char* pszTimestamp = "top genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // TopCash: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TopCashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TopCashMiner : ProcessBlock, block not accepted");
    }
